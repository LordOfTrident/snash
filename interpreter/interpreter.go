...
func cmdNotFound(cmd string, node node.Node) error {
	return err.New(node.NodeToken().Where, "Command '%v' not found", cmd)
}

func fileNotFound(path string, node node.Node) error {
	return err.New(node.NodeToken().Where, "File/directory '%v' not found", path)
}

func unexpected(node node.Node) error {
	return err.New(node.NodeToken().Where, "Unexpected %v", node.NodeToken())
}
